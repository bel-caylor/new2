		...
		echo '<div class="tpgb-panel-col tpgb-panel-col-65">';
			...
			echo '<div class="tpgb-panel-sec tpgb-p-20 tpgb-welcome-changelog tpgb-mb-8">';
				...
				echo '<div class="tpgb-changelog-list">';
					echo '<div class="changelog-date">'.esc_html__('13 April 2023','tpgb').' <span class="changelog-version">'.esc_html__('2.0.4','tpgb').'</span></div>';
					echo '<ul class="changelog-list">';
						echo '<li>'.esc_html__('Added : TP Data Table : Table Header Alignment Option','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : TP Container : Front End Tag Bug Fix','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Frontend Preview : Global Load CSS Bug Fix','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Backend Responsive Bug Fix','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Responsive Device Page Crash Bug Fix','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : More Improvements and Bugs Fixes','tpgb').'</li>';
					echo '</ul>';
				echo '</div>';

				echo '<div class="tpgb-changelog-list">';
					echo '<div class="changelog-date">'.esc_html__('03 April 2023','tpgb').' <span class="changelog-version">'.esc_html__('2.0.3','tpgb').'</span></div>';
					echo '<ul class="changelog-list">';
						echo '<li>'.esc_html__('Added : TP Search Bar : Query Include/Exclude terms from search result','tpgb').'</li>';
						echo '<li>'.esc_html__('Added : TP Pricing List : Add RichText for Price Field','tpgb').'</li>';
						echo '<li>'.esc_html__('Added : Social Review : Add Disable option for Social Icon and Profile','tpgb').'</li>';
						echo '<li>'.esc_html__('Compatibility : FSE Block Theme ','tpgb').'</li>';
						echo '<li>'.esc_html__('Compatibility : WordPress 6.2','tpgb').'</li>';
						echo '<li>'.esc_html__('Compatibility : TP Container : GeneratePress Theme','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Gravity Form : Display Rules Bug','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Frontend  Caching Performance code 80% Improvement','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Backend Editor Reusable blocks use Loop Listing improve loading Performance Editor Side','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : Accordion : Minor JS Bug','tpgb').'</li>';
						echo '<li>'.esc_html__('Fix : More Improvements and Bugs Fixes','tpgb').'</li>';
					echo '</ul>';
				echo '</div>';
