...
class Kadence_Blocks_Pro_Query_Block extends Kadence_Blocks_Pro_Abstract_Query_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector(
			'.wp-block-kadence-query' . $unique_id . ' input[type=text],' .
			'.wp-block-kadence-query' . $unique_id . ' input[type=number],' .
			'.wp-block-kadence-query' . $unique_id . ' input[type=date],' .
			'.wp-block-kadence-query' . $unique_id . ' input[type=time],' .
			'.wp-block-kadence-query' . $unique_id . ' select,' .
		...
		if ( ! empty( $query_attributes['animation'] ) && 'spinner' == $query_attributes['animation'] ) {
			$css->add_property( 'background-image', 'url("' . KBP_URL . 'src/assets/images/ajax-loader.gif");' );
		}
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$ql_query_animation = get_post_meta( $ql_id, '_kad_query_animation', true );
