...
class Kadence_Blocks_Pro_Filter_Date_Block extends Kadence_Blocks_Query_Children_Block {
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$data_attributes[] = 'data-date-format="' . ( ! empty( $attributes['dateFormat'] ) ? $attributes['dateFormat'] : '' ) . '"';
		...
		return sprintf(
			'<div class="kadence-query-filter wp-block-kadence-query-filter-date wp-block-kadence-query-filter-date%s" %s><fieldset class="kadence-filter-wrap">%s%s</fieldset></div>',
			$unique_id,
